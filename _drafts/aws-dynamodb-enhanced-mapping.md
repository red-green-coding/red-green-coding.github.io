...
In this article, we want to explore how to use it to map plain Java, Lombok, and Kotlin data classes.
...
Amazon DynamoDB is a key-value NoSQL database. Keys are of type _String_, and values can be simple types such as _String, Number, Boolean_ or
...
Fortunately, there is another option. We can use the third-party library [dynamodb-kotlin-module][dataClass-lib] that allows us to easily use data classes with DynamoDB.
...
We are using property-based testing[^2] to test our mapping comprehensively. The benefit of that approach is that
...
The test consists of a data generator. It allows us to create and populate our model classes with random values. The data generator is implemented
...
complex mapping. Our test setup will come in handy when testing the more complex mapping.
